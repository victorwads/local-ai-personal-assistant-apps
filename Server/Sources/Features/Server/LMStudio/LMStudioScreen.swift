    ...
    private var requirementsCard: some View {
        card(title: "LM Studio Requirements", subtitle: "Two server toggles can affect what you see") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("If you want tool calls from the per-request MCP bridge, enable `Allow per-request MCPs` in LM Studio Server Settings. If you want the Gmail or Calendar MCPs configured in LM Studio's `mcp.json`, also enable `Allow calling servers from mcp.json` and add `mcp/gmail` and `mcp/calendar` there.")
            }
            .font(.caption)
            .foregroundStyle(.secondary)
        }
